...
public class AStarSearch
{
    ...
    public List<Location> GetCleanPath() {
        List<Location> path = new List<Location>{goal};
        Location minLocation = goal;

        while (minLocation != start) {
            double minCost = costSoFar.ContainsKey(minLocation) ? costSoFar[minLocation] : double.MaxValue;
            bool isAdded = false;
            
            foreach (Location neighbour in graph.Neighbors(minLocation)) {
                double neighbourCost = costSoFar.ContainsKey(neighbour) ? costSoFar[neighbour] : double.MaxValue;
                if (neighbourCost < minCost) {
                    minCost = neighbourCost;
                    minLocation = neighbour;
                    path.Add(neighbour);
                    isAdded = true;
                    break;
                }
